...
try:
    ...
    # Fall back for diffusers ~0.20.0
...
class Mlp(nn.Module):
    def __init__(
        self, in_features, temb_dim=None, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.0
    ):
        super().__init__()
...
class CrossAttention(nn.Module):
    def __init__(self, dim, dim_context=None, num_heads=8, qkv_bias=False, attn_drop=0.0, proj_drop=0.0):
        super().__init__()
...
class Block(nn.Module):
    def __init__(
        ...
        self.mlp = Mlp(
            in_features=dim,
            temb_dim=temb_dim if temb_in_mlp else None,
    ...
    def forward(self, x, temb=None, mask=None, skip_connection=None):
        gate_msa, gate_mlp = (
            self.adaLN_gate(F.silu(temb)).unsqueeze(1).chunk(2, dim=-1) if hasattr(self, "adaLN_gate") else (1.0, 1.0)
        )
        shift_msa, scale_msa, shift_mlp, scale_mlp = (
            self.adaLN_modulation(F.silu(temb)).chunk(4, dim=-1) if hasattr(self, "adaLN_modulation") else 4 * [0.0]
        )
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.cross_attn = CrossAttention(
            dim, dim_context=dim_context, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop
        )
    ...
    def forward(self, x, context, temb=None, sa_mask=None, xa_mask=None, skip_connection=None):
        ...
        shift_msa, scale_msa, shift_mxa, scale_mxa, shift_mlp, scale_mlp = (
            self.adaLN_modulation(F.silu(temb)).chunk(6, dim=-1) if hasattr(self, "adaLN_modulation") else 6 * [0.0]
        )
        if self.skip_linear is not None:
            x = self.skip_linear(torch.cat([x, skip_connection], dim=-1))
        x = x + gate_msa * self.drop_path(self.self_attn(modulate(self.norm1(x), shift_msa, scale_msa), sa_mask))
        x = x + gate_mxa * self.drop_path(
            self.cross_attn(modulate(self.query_norm(x), shift_mxa, scale_mxa), self.context_norm(context), xa_mask)
        )
        x = x + gate_mlp * self.drop_path(self.mlp(modulate(self.norm2(x), shift_mlp, scale_mlp), temb))
        return x
...
class TransformerConcatCond(nn.Module):
        ...
        self.mid_block = nn.ModuleList(
            [
                Block(
                    ...
                    mlp_ratio=mid_mlp_ratio,
                    qkv_bias=mid_qkv_bias,
                    drop=mid_drop_rate,
                    attn_drop=mid_attn_drop_rate,
                    drop_path=dpr[i],
    ...
    def forward(
        self, x: torch.Tensor, temb: torch.Tensor, cond: torch.Tensor, cond_mask: torch.BoolTensor | None = None
    ) -> torch.Tensor:
        ...
        )  # Rearrange Transformer tokens to a spatial feature map for conv layers
    ...
    def __init__(
        ...
        if cond_type == "concat":
            self.mid_block = TransformerConcatCond(
                unet_dim=self.mid_dim,
                cond_dim=cond_dim,
                mid_layers=mid_layers,
                mid_num_heads=mid_num_heads,
                mid_dim=mid_dim,
                mid_mlp_ratio=mid_mlp_ratio,
                mid_qkv_bias=mid_qkv_bias,
                mid_drop_rate=mid_drop_rate,
                mid_attn_drop_rate=mid_attn_drop_rate,
                mid_drop_path_rate=mid_drop_path_rate,
                time_embed_dim=time_embed_dim,
                hw_posemb=mid_hw_posemb,
                use_long_skip=mid_use_long_skip,
            )
