...
def build_2d_sincos_posemb(h, w, embed_dim=1024, temperature=10000.0):
    ...
    """
    grid_w = torch.arange(w, dtype=torch.float) # Shape (W,)
    grid_h = torch.arange(h, dtype=torch.float) # Shape (H, )
...
def interpolate_pos_encoding(pos_embeddings: torch.Tensor, height, width, patch_size, dim_tokens) \
        -> torch.Tensor:
    """
