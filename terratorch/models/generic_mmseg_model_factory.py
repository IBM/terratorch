...
class GenericMmsegModelFactory(ModelFactory):
    def _check_model_availability(self, model, engine, **model_kwargs):
    ...
    def build_model(
        ...
        **kwargs,
        ...
        Args:
            ...
            model (str): Decoder architecture..
        ...
        try:
            ...
            self._has_mmseg = True
        except:
            engine_decoders = None
            engine_encoders = None
            self._has_mmseg = False
        ...
        backbone = self._check_model_availability(backbone, backbone_engine, **backbone_model_kwargs) 
        decoder = self._check_model_availability(decoder, decoder_engine, **decoder_model_kwargs) 

        return GenericMmsegModelWrapper(
            backbone, decoder=decoder, relu=task == "regression" and regression_relu, squeeze_single_class=task == "regression"
        )

class GenericMmsegModelWrapper(Model, nn.Module):
    def __init__(self, mmseg_model, decoder=None, relu=False, squeeze_single_class=False) -> None:
        super().__init__()
    ...
    def forward(self, *args, **kwargs):
        ...
        mmseg_output = self.mmseg_model(*args, **kwargs)
        # Decoding is optional
        input_decoder_data = [mmseg_output]
        mmseg_output_decoded = self.decode(input_data)

        if mmseg_output_decoded.shape[1] == 1 and self.squeeze_single_class:
            mmseg_output_decoded = mmseg_output_decoded.squeeze(1)

        model_output = ModelOutput(mmseg_output_decoded)

        return model_output

    def freeze_encoder(self):
        raise freeze_module(self.mmseg_model)
