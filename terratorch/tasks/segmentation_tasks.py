...
class SemanticSegmentationTask(TerraTorchTask):
    ...
    def __init__(
        ...
        self.operation_map = {
            ...
            "confidence": self.confidence_assesment,
        }
    ...
    def confidence_assesment(self, y):
        # Pixelwise difference between the first and the second most probable
        # classes
        topk = torch.topk(y, k=2, dim=1).values
        D = topk[:, 0] - topk[:, 1]

        # Negative entropy over the predicted classes
        logy = torch.log(y)
        E = torch.einsum("bijk,bijk->bjk", y, logy)

        # Percentage of high-probability pixels
        eta = 0.90
        z = y.argmax(dim=1)
        # C is still incomplete

        # Gradient statistic
        # G is still missing
        return torch.nn.Softmax()(D + E), "confidence"
