...
@pytest.mark.parametrize("config_name",["smp_resnet34_model_factory", "encoderdecoder_resnet34_model_factory"])
@pytest.mark.parametrize("checkpoint_name", ["eo_v1_100"])

def test_current_terratorch_version_buildings_predict(config_name,checkpoint_name,buildings_image):
    # Models trained with current terratorch version
    config_path = f"configs/test_{config_name}.yaml"
    pattern = os.path.join(f"/dccstor/terratorch/tmp/{checkpoint_name}/", "best-state_dict-epoch=*.ckpt")
    checkpoint_path = glob.glob(pattern)[0]

    deploy_config_path = create_deploy_config(config_path)
    update_grep_config_in_file(config_path=deploy_config_path, new_img_pattern="*.tif*")
...
@pytest.mark.parametrize("config_name", ["encoderdecoder_eo_v1_100_model_factory"])
#    ["encoderdecoder_eo_v1_100_model_factory" , "encoderdecoder_eo_v2_300_model_factory", "encoderdecoder_eo_v2_600_model_factory","prithvi_swinB_model_factory_config", "test_prithvi_swinL_model_factory_config","smp_resnet34_model_factory_config","encoderdecoder_timm_resnet34_model_factory"])
@pytest.mark.parametrize("checkpoint_name", ["eo_v1_100"])
#  "eo_v2_300", "eo_v2_600", "swinb", "swinl"
# Models trained with current terratorch version
def test_current_terratorch_version_burnscars_predict(config_name, checkpoint_name, burnscars_image):
    config_path = f"configs/test_{config_name}.yaml"

    deploy_config_path = create_deploy_config(config_path)
    print(f"config_path : {deploy_config_path}")
    update_grep_config_in_file(config_path=deploy_config_path, new_img_pattern="*.tif*")
    ...
    # checkpoint_path = f"/dccstor/terratorch/tmp/{model_name}/best-state-dict-epoch=*.ckpt"

    preds = run_inference(config=deploy_config_path, checkpoint=checkpoint_path, image=burnscars_image)
...
def cleanup(model_name):
    # Delete all checkpoints after running inference.
    # Delete all config_deploy yamls. 
