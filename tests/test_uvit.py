...
def test_block_variants():
    ...
    # without temb
    blk2 = Block(
        dim=8, num_heads=2, temb_dim=None, skip=False, temb_in_mlp=False, temb_gate=False, temb_after_norm=False
    )
    out2 = blk2(x)
...
def test_transformer_concat_cond_mask_and_longskip():
    model = TransformerConcatCond(
        unet_dim=8, cond_dim=4, mid_layers=3, mid_num_heads=2, mid_dim=8, hw_posemb=4, use_long_skip=True
    )
...
def test_transformer_xattn_cond_mask_longskip():
    model = TransformerXattnCond(
        unet_dim=8, cond_dim=8, mid_layers=3, mid_num_heads=2, mid_dim=8, hw_posemb=4, use_long_skip=True
    )
    x = torch.randn(1, 8, 4, 4)
    temb = torch.randn(1, 512)
    cond = torch.randn(1, 8, 4, 4)
...
def test_weight_init_embedding_std():
    ...
    if hasattr(model, "init_std"):
        assert hasattr(model, "init_std") and math.isclose(model.init_std, 0.02, rel_tol=1e-3)
